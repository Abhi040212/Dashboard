...
# Enhanced data loading with better error handling - UPDATED URL
...
with st.sidebar:
    ...
    # Data source selection - DEFAULT TO GOOGLE SHEETS
    data_source = st.radio(
        "**📊 Data Source:**",
        ["Google Sheets (Auto-Update)", "Upload Excel File"],
        index=0,  # This sets Google Sheets as default
...
if df is not None and filtered_df is not None:
    ...
    if not filtered_df.empty:
        ...
        # Enhanced data table - keep original structure but show all columns
    ...
    if not filtered_df.empty:
        ...
        # Continue with other analytics sections (Source, Team, AE, Time-based)...
